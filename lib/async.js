...
(function() {
  var Promise, fs, ContentTypes, DocPropsApp, JSZip, SharedStrings, Sheet, Style, Workbook, XlRels, XlWorkbook, baseXl, opt, tool, xml,
  ...
  Workbook = (function() {
    ...
    Workbook.prototype.save = function(cb) {
      // TODO: add ability to specify filename+path here
      var target = this.fpath + '/' + this.fname;
      return this.generate().then(function(zip) {
        var buffer = zip.generate({type: 'nodebuffer'});
        return fs.writeFileAsync(target, buffer);
      }).asCallback(cb);
    };
    ...
    Workbook.prototype.generate = function(cb) {
      ...
      return Promise.resolve().then(function(){
        ...
        zip.file('[Content_Types].xml', self.ct.toxml());
        zip.file('docProps/app.xml', self.da.toxml());
        zip.file('xl/workbook.xml', self.wb.toxml());
        zip.file('xl/sharedStrings.xml', self.ss.toxml());
        zip.file('xl/_rels/workbook.xml.rels', self.re.toxml());
        for (i = _i = 0, _ref = self.sheets.length; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
          zip.file('xl/worksheets/sheet' + (i + 1) + '.xml', self.sheets[i].toxml());
        }
        zip.file('xl/styles.xml', self.st.toxml());
        return zip;
      }).asCallback(cb);
    };
